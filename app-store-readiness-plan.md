...
| 6 | Missing 3D assets: code loads `art.scnassets/player.usdc` and `art.scnassets/BUTTERFLY.usdz`, but no `art.scnassets` folder exists. The game silently falls back to a white capsule and gray cubes. `model.glb` (8.7 MB) sits at the project root, but SceneKit cannot load GLB — it must be converted to USDZ. | High |
...
5. **Add the 3D models.** Create `art.scnassets/`, convert `model.glb` to `player.usdz`/`BUTTERFLY.usdz` (Reality Converter or `usdzconvert`), drop the GLB from the iOS target, and verify both models load (no capsule/cube fallback).
6. **Clean the project.** Delete `File.txt`, `Untitled.swift`, `Property List.plist`, the stray `model.glb` and unused `logo.gif`; `git rm --cached` the committed `xcuserdata` files.
...
6. **Third-party content check:** the name "STARBUGS", the Jedi-style font, and any Star Wars-adjacent branding are rejection risks under guideline 5.2 (intellectual property). Audit names/assets and rename/replace anything that trades on someone else's IP.
